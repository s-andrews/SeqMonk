		...
		try {
			...
			for (int d=0;d<visibleHiCDataSets.length;d++) {
				...
				if (baits.length == 1) {
					...
					newData[d] = new DataSet(prefs.prefix.getText()+"_"+visibleHiCDataSets[d].name(),"HiC other end of "+visibleHiCDataSets[d].name()+" for region "+baits[0].chromosome().name()+":"+baits[0].start()+"-"+baits[0].end(),DataSet.DUPLICATES_REMOVE_NO,prefs.getImportOptionsDescription());
				}
				else if (mergeIntoOne){
					System.err.println("Multiple baits, but merging");
					newData[d] = new DataSet(prefs.prefix.getText()+"_"+visibleHiCDataSets[d].name(),"HiC other end of "+visibleHiCDataSets[d].name()+" for "+baits.length+" regions",DataSet.DUPLICATES_REMOVE_NO,prefs.getImportOptionsDescription());
				}
				...
				for (int b=0;b<baits.length;b++) {
					...
					if (!(baits.length == 1 || mergeIntoOne)) {
						newData[(d*baits.length)+b] = new DataSet(baits[b].toString()+"_"+visibleHiCDataSets[d].name(),"HiC other end of "+visibleHiCDataSets[d].name()+" for region "+baits[b].chromosome().name()+":"+baits[b].start()+"-"+baits[b].end(),DataSet.DUPLICATES_REMOVE_NO,prefs.getImportOptionsDescription());						
					}
